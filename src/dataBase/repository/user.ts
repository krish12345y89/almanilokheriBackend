...
export class UserRepository {
  async userSignUp(data: IUser, next: NextFunction): Promise<IUser | void> {
    try {
      if (data) {
  ...
  async searchUser(data: searchUserType, next: NextFunction) {
    try {
      if (!data) {
  ...
  async userUpdateForPending(
    _id: string,
    data: updatePendingUser,
    next: NextFunction
  ): Promise<IUser | void> {
    try {
      if (!data || !_id) {
  ...
  async userDelete(uuid: string, next: NextFunction): Promise<void> {
    try {
