...
export const insertDeviceRecord = async (req: Request, res: Response) => {
...
export const getDeviceRecord = async (req: Request, res) => {
  try {
    const id = process.env.PrivetKey;
...
export const insertdeviceDailyRecord = async (req: Request, res: Response) => {
  const { DeviceId } = req.body;
  try {
    if (isUUID(DeviceId)) {
      const now = new Date();
