...
const validateFileExtension = (file: any, allowedExtensions: string[]) => {
  if (!file) {
  ...
  if (!allowedExtensions.includes(fileExtension)) {
    throw new Error(
      `File must be one of the following types: ${allowedExtensions.join(", ")}`
    );
  }
};

// Check for duplicate field in the database
const checkDuplicate = (field: string): CustomValidator => {
