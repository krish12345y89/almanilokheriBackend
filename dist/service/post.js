...
export class PostService {
    ...
    async updatePostService(id, title, description, removedImages, files, next) {
        try {
            ...
            let imagesToDelete = [];
            let newImageFiles = files || [];
            let updatedDocuments = [...post.documents];
            if (removedImages) {
                try {
                    const removedImageIndices = JSON.parse(removedImages);
    ...
    async deletePost(id, next) {
        try {
            ...
            if (post.documents) {
                await deleteS3Files(post.documents.map(({ file }) => file), next);
            }
