...
export const newGroupChat = async (req, res, next) => {
    try {
        ...
        console.log(error);
        return next(new ErrorHandle("failed to create  group chat , please try again or try some time later", 500));
    }
};
...
export const deleteMessage = async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return next(new ErrorHandle("please provide chatId, messageId and id", 400));
        }
        const { chatId } = req.params;
        if (!chatId)
            return next(new ErrorHandle("please provide chatId", 400));
...
export const updateMessage = async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return next(new ErrorHandle("please provide chatId, messageId, id and message", 400));
        }
        const { chatId } = req.params;
        if (!chatId)
            return next(new ErrorHandle("please provide chatId", 400));
        ...
        const { message } = req.body;
        if (!message)
            return next(new ErrorHandle("please provide message", 400));
