...
export const errorHandler2 = async (err, next) => {
    try {
        ...
        if (err.name === "MongoServerError" || err.name === "MongooseError") {
            if (err.code === 11000 || err.cause?.code === 11000) {
                ...
                next(new ErrorHandle(`Duplicate key error: ${keyValues} already exists`, 400, false, true));
                return;
            }
            ...
            next(new ErrorHandle(`Database error: An unknown error occurred`, 500, false, true));
            return;
        }
        ...
        if (err.name === "CastError") {
            ...
            next(new ErrorHandle(`Invalid value for field "${err.path}"`, 400, false, true));
            return;
        }
        // Handle MissingSchemaError
        if (err.name === "MissingSchemaError") {
            console.error("Schema Error:", err);
            next(new ErrorHandle(`Schema for "${err.name}" is missing`, 500, false, true));
            return;
        }
