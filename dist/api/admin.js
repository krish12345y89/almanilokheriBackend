...
app.get("/userCounts", await userAuth.isAuthorised(), async (req, res, next) => {
    try {
        const adminVerify = await AdminUser.findById(req.user);
        if (!adminVerify) {
            return next(new ErrorHandle("only admin users can process this routes", 401));
        }
...
app.post("/login", async (req, res, next) => {
    try {
        ...
        if (!userName || !password) {
            return next(new ErrorHandle("please provide both username and password", 400));
        }
...
app.post("/signUp", async (req, res, next) => {
    try {
        ...
        if (adminExist) {
            return next(new ErrorHandle("user already exists with this details", 400));
        }
