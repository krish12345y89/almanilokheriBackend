...
]), userSignUpValidation, validateErrors, async (req, res, next) => {
    try {
        const data = req.body;
        const ip = req.ip;
        data.ip = ip;
        ...
        if (!proofFile && !data.proof) {
            return next(new ErrorHandle("Please provide either proof or a referral user to validate yourself", 400, false, true));
        }
        let proofData;
        if (proofFile) {
            ...
            return next(new ErrorHandle("Proof must be either a valid file or a referral ObjectId", 400, false, true));
        }
        if (!avatar) {
            return res
                .status(400)
...
app.put("/updatePending/:_id", upload.fields([
    { name: "avatar", maxCount: 1 },
    { name: "proof", maxCount: 1 },
]), async (req, res, next) => {
    try {
        ...
        const proofFile = req.files?.["proof"]?.[0];
        if (!_id) {
            return next(new ErrorHandle("Please signIn first to update yourself", 400, false, true));
        }
        if (!data) {
            return next(new ErrorHandle("Please provide any field to update", 400, false, true));
        }
        if (proofFile) {
        ...
        if (data.proof) {
            if (!mongoose.Types.ObjectId.isValid(data.proof) && !proofFile) {
                return next(new ErrorHandle("Please provide a valid proof", 400, false, true));
            }
        }
        if (avatar) {
...
app.put("/updateUser/:_id", upload.fields([
    {
        name: "avatar",
        maxCount: 1,
    },
...
app.put("/updateStatus", upload.none(), async (req, res, next) => {
    try {
        ...
        console.error(error);
        next(new ErrorHandle("failed update user update status at api level", 500));
    }
});
export default app;
